...
export default () => {
  ...
  const [longActiveDeveloperCount, setLong] = useState([]);
  ...
  useEffect(() => {
    fetch('https://mixpanel.com/api/2.0/insights?bookmark_id=14774375', {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        Authorization: 'Basic NzEwMGI4ZGM1MTE4NmFlZGRjZmQ5ZGU3ZGExNzExNjk6',
  ...
  const currentMonthly =
    monthlyActiveDeveloperCount[monthlyActiveDeveloperCount.length - 1];
  const prevMonthly =
    monthlyActiveDeveloperCount[monthlyActiveDeveloperCount.length - 8];
  const currentWeekly =
    weeklyActiveDeveloperCount[weeklyActiveDeveloperCount.length - 1];
  const prevWeekly =
    weeklyActiveDeveloperCount[weeklyActiveDeveloperCount.length - 8];

  return (
    <div>
      <h3>
        Monthly Active Developer(Internal){' '}
        <Tooltip text={term.monthly_active_developer} />:{' '}
        <strong className="green">{currentMonthly}</strong>
        {prevMonthly && <Diff current={currentMonthly} prev={prevMonthly} />}
      </h3>
      <h3>
        Weekly Active Developer(Internal){' '}
        <Tooltip text={term.weekly_active_developer} />:{' '}
        <strong className="green">{currentWeekly}</strong>
